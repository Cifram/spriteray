	...
	where F: Fn(Vec3) -> SdfResult
{
	...
	for y in 0..height {
		for x in 0..width {
			...
			if let Some(color) = raycast(sdf, origin, direction, max_range) {
...
fn raycast<F>(
	sdf: &F, ray_start: Vec3, ray_direction: Vec3, ray_length: f32
) -> Option<Color>
	where F: Fn(Vec3) -> SdfResult
{
	let mut len = 0.0;
	while len < ray_length {
		match sdf(ray_start + ray_direction * len) {
			SdfResult::Hit { range: _, normal: _, color } => return Some(color),
